...
cat_features = ['sex']
print('train features nums: ',len(train_features))
...
###############################################################
model = SBBTree(task='S1',params=params, stacking_num=5, bagging_num=3,  bagging_test_size=0.33, num_boost_round=10000, early_stopping_rounds=200)
...
# 保存训练集和测试集
TrainFeatures.data_BuyOrNot_FirstTime.to_csv('../data/train.csv')
PredFeatures.data_BuyOrNot_FirstTime.to_csv('../data/Xtest.csv')
...
model = SBBTree(task='S2',params=params, stacking_num=5, bagging_num=3,  bagging_test_size=0.33, num_boost_round=10000, early_stopping_rounds=200)
...
S2 = model.S2
out_submit.head(50000).to_csv('../submit/predict_S2_%.4f.csv'%S2,index=False,header=True)
